...
export const TCRP_FEATURES: TcrpFeature[] = [
  ...
  { id: "tool-call-coalescing", name: "Tool-Call Coalescing Gate", surface: "Hook", ref: "tool-call-coalescing.mjs", category: "cost-security", icon: "🪢", description: "Advises when the SAME tool + canonical input is dispatched twice inside one turn's parallel block — both copies bill, one is waste (L4-27)." },
  { id: "billing-tier-drift", name: "Billing-Tier Drift", surface: "Hook", ref: "billing-tier-drift.mjs", category: "cost-security", icon: "📈", description: "Detects a mid-session usage.service_tier flip (string equality; absent tier = no signal); differential priced only when both tier rates are known, else null (L4-35)." },
  ...
  { id: "repo-map", name: "Repo Map", surface: "MCP tool", ref: "repo_map", category: "context-selection", icon: "🗺️", description: "Aider-style PageRank symbol map, signatures only; fitted to a token budget by binary search with locally measured sizes (prune-proof map renders it on screen and persists it; seeds the governed arm's context brief). TS/JS in v0.1, stated honestly on other repos." },
  ...
  { id: "repo-proof", name: "Repo-Proof (f20)", surface: "MCP tool", ref: "repo_proof_status", category: "integrity", icon: "🧾", description: "Evidence-gated repo-local proof: mines tasks from the repo's history, three-state-verifies them, runs the paired matrix under a budget, and promotes flags only on a passing Ed25519-attested proof. Operator lifecycle via the prune-proof CLI; this read-only tool reports state + flag provenance." },
  { id: "knowledge-compiler", name: "Knowledge Compiler (f21)", surface: "MCP tool", ref: "knowledge_compile", category: "learning", icon: "🏗️", description: "Compiles the repo into a deterministic, content-SHA-keyed asset (symbols, dependency edges, CODEOWNERS ownership, churn signals); incremental; truncation and skipped CODEOWNERS lines reported honestly." },
  { id: "asset-store", name: "Proof-Carrying Asset Store (f22)", surface: "MCP tool", ref: "memory_search", category: "learning", icon: "🗄️", description: "Verified Repo Memory: provenance-mandatory entries re-validated on every read (stale entries self-demote naming moved files), contradiction demotes the superseded entry, sentinel-screened writes; memory_search/get/store/validate." },
];
